...
db_roupas = 'cadastro_roupas.json'

def carregar_dados():
    if os.path.exists(db_roupas):
        with open(db_roupas, "r", encoding="utf-8") as arq_json:
...
def obter_dados():
    ...
    while True:
        ...
        if not nome_roupa or not descricao_roupa or not tamanho or not cor or not valor or not quantidade:
...
def cadastrar_roupa(receber_roupa):
    ...
    with open(db_roupas, "w", encoding="utf-8") as arq_json:
